...
def test_raw_quotes_create_notional_features_and_keep_execution_units(tmp_path, unit, executor_kind):
    source, output = tmp_path / 'raw.duckdb', tmp_path / 'episodes'
    raw_database(source, unit)
    assert extractor.extract_data(str(source), 'datasets', str(output), 1, 27, 2,
